...
export const MRT_TableRoot: any = <TData extends Record<string, any> = {}>(
    ...
    const initialState: Partial<MRT_TableState<TData>> = useMemo(() => {
        ...
        initState.columnOrder = initState.columnOrder ?? getDefaultColumnOrderIds(props)
    ...
    const [columnFilterFns, setColumnFilterFns] = useState<MRT_FilterFnsState>(() =>
        Object.assign(
            {},
            ...getAllLeafColumnDefs(props.columns as MRT_ColumnDef<TData>[]).map((col) => ({
                [getColumnId(col)]:
    ...
    const [draggingColumn, setDraggingColumn] = useState<MRT_Column<TData> | null>(initialState.draggingColumn ?? null)
    const [draggingRow, setDraggingRow] = useState<MRT_Row<TData> | null>(initialState.draggingRow ?? null)
    const [editingCell, setEditingCell] = useState<MRT_Cell<TData> | null>(initialState.editingCell ?? null)
    const [editingRow, setEditingRow] = useState<MRT_Row<TData> | null>(initialState.editingRow ?? null)
    const [globalFilterFn, setGlobalFilterFn] = useState<MRT_FilterOption>(initialState.globalFilterFn ?? 'fuzzy')
    const [grouping, setGrouping] = useState<MRT_GroupingState>(initialState.grouping ?? [])
    const [hoveredColumn, setHoveredColumn] = useState<MRT_Column<TData> | { id: string } | null>(
        initialState.hoveredColumn ?? null,
    )
    const [hoveredRow, setHoveredRow] = useState<MRT_Row<TData> | { id: string } | null>(
        initialState.hoveredRow ?? null,
    )
    const [isFullScreen, setIsFullScreen] = useState(initialState?.isFullScreen ?? false)
    const [showAlertBanner, setShowAlertBanner] = useState(props.initialState?.showAlertBanner ?? false)
    ...
    const displayColumns = useMemo(
        () =>
            (
                [
                    (props.state?.columnOrder ?? columnOrder).includes('mrt-row-drag') && {
                    ...
                    (props.state?.columnOrder ?? columnOrder).includes('mrt-row-actions') && {
                        Cell: ({ cell, row }) => (
                    ...
                    (props.state?.columnOrder ?? columnOrder).includes('mrt-row-expand') &&
                        showExpandColumn(props, props.state?.grouping ?? grouping) && {
                            ...
                            Header: props.enableExpandAll ? () => <MRT_ExpandAllButton table={table as any} /> : null,
                    ...
                    (props.state?.columnOrder ?? columnOrder).includes('mrt-row-select') && {
                        Cell: ({ row }) => (
                            <MRT_SelectCheckbox
                                row={row as any}
                                table={table as any}
                    ...
                    (props.state?.columnOrder ?? columnOrder).includes('mrt-row-numbers') && {
    ...
    const columnDefs = useMemo(
        ...
        [columnFilterFns, displayColumns, props.columns, props.state?.columnFilterFns],
    )
    ...
    const data: TData[] = useMemo(
        () =>
            (props.state?.isLoading || props.state?.showSkeletons) && !props.data.length
                ? [
                      ...Array(props.state?.pagination?.pageSize || initialState?.pagination?.pageSize || 10).fill(
                          null,
                      ),
                  ].map(() =>
                      Object.assign(
                          {},
                          ...getAllLeafColumnDefs(props.columns as MRT_ColumnDef[]).map((col) => ({
    ...
    const table = {
        ...useReactTable({
            ...
            columns: columnDefs,
            data,
            globalFilterFn: props.filterFns?.[globalFilterFn] ?? props.filterFns?.fuzzy,
            initialState,
            state: {
                columnFilterFns,
                columnOrder,
    ...
    useEffect(() => {
        ...
        const totalRowCount = props.rowCount ?? table.getPrePaginationRowModel().rows.length
