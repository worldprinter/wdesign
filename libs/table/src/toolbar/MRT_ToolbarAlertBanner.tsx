        ...
        >
            <Alert
                ...
                sx={(theme) => ({
                    ...
                    left: 0,
                    position: 'relative',
                    marginBottom: stackAlertBanner ? 0 : positionToolbarAlertBanner === 'bottom' ? '-16px' : undefined,
                    padding: '8px',
                    right: 0,
                    top: 0,
                    width: '100%',
                    zIndex: 2,
                    ...(alertProps?.sx instanceof Function ? alertProps.sx(theme) : (alertProps?.sx as any)),
                })}
            >
                {alertProps?.title && <Box>{alertProps.title}</Box>}
                <Flex sx={{ padding: '8px 16px' }}>
                    {alertProps?.children}
                    {alertProps?.children && (selectMessage || groupedByMessage) && <br />}
                    {selectMessage}
