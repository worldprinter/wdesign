...
export function useActionsState({ actions, onActionsChange }: UseActionsState) {
    const registerActions = (payload: SpotlightAction[]) => onActionsChange?.(prepareActions([...actions, ...payload]))

    const removeActions = (ids: string[]) => onActionsChange?.(actions.filter((action) => !ids.includes(action.id)))
