...
export const NumberInput = forwardRef<HTMLInputElement, NumberInputProps>((props, ref) => {
    ...
    const onStepLoop = (isIncrement: boolean) => {
        ...
        if (shouldUseStepInterval) {
            const interval =
                typeof stepHoldInterval === 'number' ? stepHoldInterval : stepHoldInterval(stepCountRef.current)
            onStepTimeoutRef.current = window.setTimeout(() => onStepLoop(isIncrement), interval)
        }
    }

    const onStep = (event: React.MouseEvent<HTMLElement> | React.KeyboardEvent<HTMLElement>, isIncrement: boolean) => {
    ...
    return (
        <TextInput
            ...
            onKeyUp={handleKeyUp}
            rightSection={
                rightSection || (disabled || readOnly || hideControls || variant === 'unstyled' ? null : controls)
            }
            rightSectionWidth={rightSectionWidth ?? `calc(${getSize({ size, sizes: CONTROL_SIZES })} + ${rem(1)})`}
            radius={radius}
            max={max}
            min={min}
            step={step}
