...
export const MRT_CopyButton = <TData extends Record<string, any> = {}>({ cell, children, table }: Props<TData>) => {
    const {
        ...
        <CopyButton value={cell.getValue<string>()}>
            {({ copied, copy }) => (
                <Tooltip
                    ...
                    openDelay={1000}
                    position='top'
                    label={buttonProps?.title ?? (copied ? localization.copiedToClipboard : localization.clickToCopy)}
                >
                    <UnstyledButton
                        {...buttonProps}
                        onClick={(e) => {
                        ...
                        sx={(theme) => ({
                            ...
                            ...(buttonProps?.sx instanceof Function ? buttonProps.sx(theme) : (buttonProps?.sx as any)),
                        })}
                        title={undefined}
                    >
                        {children}
