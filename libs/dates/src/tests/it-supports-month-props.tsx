...
export function itSupportsMonthProps(Component: React.FC<MonthTestProps>, requiredProps?: Record<string, any>) {
    ...
    it('supports renderDay', () => {
        const { container } = render(
            <Component
                {...requiredProps}
                renderDay={(date) => date.getFullYear()}
            />,
        )
        const days = getDays(container)
    ...
    it('allows changing days aria-label with getDayAriaLabel prop', () => {
        const { container } = render(
            <Component
                {...requiredProps}
                getDayAriaLabel={(date) => dayjs(date).format('DD/MM/YYYY')}
            />,
        )
